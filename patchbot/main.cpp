...
int main(int argc, char* argv[]) {
	...
	std::string i_path = argv[1];
	std::string o_path = argv[2];

	/*const char* i_path = "..\\..\\roboter\\patchbot.tga";
	const char* o_path = "..\\..\\roboter\\dead_out.tga";*/

	try {
		std::cout << "Reading image from TARGA file at " << i_path << "... " 
			<< std::endl;
		tga img = pb_input::read_tga_img(i_path);
		std::cout << "Read image successfully." << std::endl << std::endl;

		std::cout << "Writing TARGA image file to " << o_path << "... " 
			<< std::endl;
		pb_output::write_tga_img(o_path, img);
		std::cout << "Wrote image successfully." << std::endl;
	}
