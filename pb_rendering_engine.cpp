...
int rendering_engine::get_full_width_px()
{
    return parent->map.get_width()
        * resources.get_terrain_img.at(parent->map.get_tile(0, 0)->get_terrain()).width();
}

int rendering_engine::get_full_height_px()
{
    return parent->map.get_height()
        * resources.get_terrain_img.at(parent->map.get_tile(0, 0)->get_terrain()).height();
}
...
void rendering_engine::do_initial_render()
{
    ...
    QPainter qpaint(&pixmap);

    for (int y = 0; y < pixmap.height() / 32; y++) {
        for (int x = 0; x < pixmap.width() / 32; x++) {
            qpaint.drawPixmap(x * 32, y * 32, resources.get_terrain_img.at(terrain::MAIN_SERVER));
        }
    }
